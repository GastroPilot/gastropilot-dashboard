...
const normalizeStatus = (value: string): WaitlistStatusValue | null => {
  ...
  const allowed: Reservation['status'][] = [
    'pending',
    'confirmed',
    'seated',
    'completed',
  ...
  return allowed.includes(normalized as Reservation['status'])
...
export function WaitlistSidebar({
  ...
  const allFilterKeys = useMemo(
    () => Object.keys(waitlistFilters) as WaitlistFilterKey[],
    [waitlistFilters]
  );
  ...
  const filteredReservations = useMemo(() => {
    ...
    const activeStatusSet = new Set(
      activeStatusKeys.flatMap((key) => waitlistFilters[key].statuses)
    );
  ...
  const activeStatusKeys = selectedStatuses.filter(
    (key) => key !== 'block'
  ) as WaitlistStatusFilterKey[];
  const activeReservationKeys = selectedStatuses.length > 0 ? activeStatusKeys : statusFilterKeys;
            ...
            <div className="text-center text-gray-400 py-6 md:py-8">
              ...
              <p className="text-sm">
                {searchQuery.trim()
                  ...
                  : 'Keine Reservierungen in der Warteliste'}
              </p>
              {isOver && (
