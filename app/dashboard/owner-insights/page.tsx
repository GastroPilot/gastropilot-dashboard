...
export default function OwnerInsightsPage() {
  ...
  const computeKpis = (list: Reservation[], windowRange: { from: Date; to: Date }) => {
    ...
    const confirmed = list.filter(
      (r) => ['confirmed', 'seated', 'completed'].includes(r.status) && r.table_id !== null
    ).length;
    ...
    const durations = list
      .filter((r) => r.end_at)
      .map((r) => differenceInMinutes(parseISO(r.end_at), parseISO(r.start_at)));
    const lead = list
      .filter((r) => r.created_at_utc)
      .map((r) => differenceInHours(parseISO(r.start_at), parseISO(r.created_at_utc)));
    ...
    const avgSeats = bucketValues.length
  ...
  const kpis = useMemo(
    () => computeKpis(filteredReservations, range),
    [filteredReservations, range, tables]
  );
  const prevKpis = useMemo(
    () => computeKpis(prevFilteredReservations, prevRange),
    [prevFilteredReservations, prevRange, tables]
  );
  ...
  const renderDelta = (value: number, isPercent = false) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 text-xs ${positive ? 'text-emerald-400' : 'text-amber-400'}`}
      >
  ...
  const timeBuckets = useMemo(
    () =>
      [
        {
          label: 'Früh (bis 12 Uhr)',
          value: filteredReservations.filter((r) => parseISO(r.start_at).getHours() < 12).length,
        },
        {
        ...
        {
          label: 'Spät (ab 21 Uhr)',
          value: filteredReservations.filter((r) => parseISO(r.start_at).getHours() >= 21).length,
        },
      ] as const,
    [filteredReservations]
  );
  ...
  const handleExportPDF = () => {
    ...
    if (iframe.contentWindow) {
      ...
      iframe.contentWindow.document.write(
        `<!doctype html><html><head>${styles}</head><body>${contentHtml}</body></html>`
      );
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      rows: [
        {
          label: 'Bestätigt/platziert/abgeschlossen (mit Tisch)',
          value: `${percentFormatter.format(kpis.confirmationRate)}%`,
        },
        {
          label: 'Tisch-Zuordnung',
          value: `${percentFormatter.format(kpis.tableAssignmentRate)}%`,
        },
    ...
    },
    {
      ...
      delta:
        kpis.noShowRate + kpis.cancellationRate - (prevKpis.noShowRate + prevKpis.cancellationRate),
      deltaIsPercent: true,
      rows: [
  ...
  return (
    <div className="h-full min-h-screen overflow-auto bg-gray-900 text-gray-100">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                <span>
                  Kennzahlen aus Reservierungen, Gästen, Kanälen, Status-Pipeline und aktiven
                  Tischen.
                </span>
              </div>
            </div>
      ...
      <div
        id="owner-insights-content"
        className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24 space-y-6"
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="relative col-span-1" ref={rangeMenuRef}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 transition-transform ${rangeMenuOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                {rangeMenuOpen && (
                      ...
                      <span className="flex items-center gap-2 truncate">
                        {SelectedIcon && (
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-md border border-gray-600 bg-gray-900/70">
                            <SelectedIcon
                              className={`w-3.5 h-3.5 ${selected?.tone ?? 'text-gray-300'}`}
                            />
                          </span>
                        )}
                          ...
                          <span className="flex items-center gap-2">
                            {opt.Icon && (
                              <span className="inline-flex items-center justify-center w-6 h-6 rounded-md border border-gray-700 bg-gray-900/70">
                                <opt.Icon
                                  className={`w-3.5 h-3.5 ${opt.tone ?? 'text-gray-300'}`}
                                />
                              </span>
                            )}
                ...
                >
                  <CardHeader className="pb-1">
                    <CardTitle className="text-sm text-gray-300 flex items-center gap-2">
                      <span
                        className={`p-1.5 rounded-lg bg-white/5 border border-white/5 ${card.accent}`}
                      >
                        {card.icon}
                      </span>
                      {card.title}
                  ...
                  <CardContent className="pt-0">
                    <div className="flex items-baseline gap-2">
                      ...
                      {typeof card.delta !== 'undefined' &&
                        renderDelta(card.delta, card.deltaIsPercent)}
                    </div>
                    <div className="text-sm text-gray-400">{card.sub}</div>
                    {card.spark.length > 0 && (
                      <Sparkline values={card.spark} colorClass={card.accent} />
                    )}
                  </CardContent>
                </Card>
              ))}
                ...
                <CardContent className="pt-1 space-y-2">
                  {dailySeries.length ? (
                    ...
                    <div className="text-sm text-gray-400 py-4">
                      Keine Reservierungen im Zeitraum.
                    </div>
                  )}
                </CardContent>
              </Card>
                  ...
                  setDetail({
                    ...
                    rows: Object.entries(statusBreakdown).map(([status, count]) => {
                      ...
                      return {
                        ...
                        value: `${count} · ${percentFormatter.format(share)}%`,
                      };
                    }),
                  });
                  setDetailOpen(true);
              ...
              >
                ...
                <CardContent className="pt-1 space-y-2">
                  ...
                  <div className="text-xs text-gray-400 pt-2 border-t border-gray-800">
                    Tischzuordnung {percentFormatter.format(kpis.tableAssignmentRate)}% · Ø Dauer{' '}
                    {kpis.avgDuration.toFixed(0)} Min.
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                onClick={() => {
                  setDetail({
                    ...
                    rows: channelBreakdown.map(([channel, count]) => {
                      const share = filteredReservations.length
                    ...
                    latestReservations.map((res) => (
                      <div key={res.id} className="p-3 rounded-lg bg-white/5 border border-white/5">
                        ...
                        <div className="text-xs text-gray-400">
                          {format(parseISO(res.start_at), 'dd.MM.yyyy HH:mm', { locale: de })} ·{' '}
                          {res.channel || 'Unbekannt'}
                        </div>
                        <div className="text-xs text-gray-400">
                          Status: {res.status}{' '}
                          {res.table_id ? `· Tisch ${res.table_id}` : '· ohne Tisch'}
                        </div>
                      </div>
                    ))
                  ) : (
                ...
                onClick={() => {
                  setDetail({
                    ...
                    rows: timeBuckets.map(({ label, value }) => {
                      const share = filteredReservations.length
                        ? (value / filteredReservations.length) * 100
                        : 0;
              ...
              >
                ...
                <CardContent className="pt-1 space-y-2">
                  ...
                  <div className="text-xs text-gray-400 pt-2 border-t border-gray-800">
                    Ø Vorlaufzeit {kpis.avgLead.toFixed(1)}h · Ø Dauer {kpis.avgDuration.toFixed(0)}{' '}
                    Min.
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="pt-1 space-y-2">
                  {slotBreakdown.length ? (
                    slotBreakdown.map((slot) => (
                      <div
                        key={slot.ts}
                        className="flex items-center justify-between text-sm text-gray-300 py-1 border-b border-white/5 last:border-b-0"
                      >
          ...
          {detail && (
            <DialogContent className="bg-gray-900 border border-gray-700">
              <DialogHeader>
                ...
                <DialogDescription>
                  Zeitraum: {format(range.from, 'dd.MM.yyyy', { locale: de })} –{' '}
                  {format(range.to, 'dd.MM.yyyy', { locale: de })} · Filter:{' '}
                  {statusFilter === 'all' ? 'alle Status' : statusFilter}
                </DialogDescription>
              </DialogHeader>
