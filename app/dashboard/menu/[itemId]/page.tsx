...
export default function EditMenuItemPage() {
  ...
  if (errorMessage || !restaurant || !itemId) {
    return (
      <div className="p-6">
        <p className="text-muted-foreground">{errorMessage ?? 'Gericht nicht gefunden.'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-background">
      <div className="mx-auto max-w-3xl p-4 md:p-6">
        ...
        <h1 className="mb-6 text-2xl font-semibold text-foreground">Gericht bearbeiten</h1>
        <MenuItemEditor restaurantId={restaurant.id} itemId={itemId} />
      </div>
    </div>
  );
}
