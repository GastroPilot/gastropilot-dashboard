  ...
  useEffect(() => {
    const handleNFCLogin = async () => {
      ...
      if (!tagId) {
        setStatus('error');
        setMessage(
          'Keine Tag-ID in der URL gefunden. Bitte scannen Sie den NFC-Transponder erneut.'
        );
        return;
      }
      ...
      try {
        ...
        if (err instanceof ApiError) {
          if (err.status === 401) {
            setMessage(
              'Ungültiger NFC-Transponder. Dieser Transponder ist keinem Benutzer zugeordnet.'
            );
...
export default function NFCLoginPage() {
  return (
    <Suspense fallback={<LoadingOverlay message="Wird geladen..." />}>
      <NFCLoginContent />
    </Suspense>
  );
}
