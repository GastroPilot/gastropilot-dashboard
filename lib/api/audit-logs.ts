...
type AuditLogListApiResponse = AuditLog[] | {
  results?: AuditLog[];
  items?: AuditLog[];
  data?: AuditLog[];
  total?: number;
...
const normalizeAuditLogResponse = (
  ...
  const offset = typeof data.offset === "number"
    ? data.offset
    : (data.page && limit ? (data.page - 1) * limit : fallbackOffset);
  const hasTotal = data.total !== undefined || data.total_count !== undefined || data.count !== undefined;
  const total = data.total ?? data.total_count ?? data.count ?? Math.max(results.length + offset, 0);
...
export const auditLogsApi = {
  list: async (restaurantId: string, params: AuditLogFilters = {}): Promise<AuditLogListResponse> => {
    ...
    const url = queryString ? `/restaurants/${restaurantId}/audit-logs/?${queryString}` : `/restaurants/${restaurantId}/audit-logs/`;
